---
layout: post
title: "Patricia Tree (PAT tree)"
date: 2007-04-17 03:09:38 +0800
tags: code
highlightjs:
  - latex
---

[Patricia tree](http://en.wikipedia.org/wiki/Patricia_tree) is a tree that implements the idea of:

which is a radix tree to store strings. It operates in $$O(k)$$ time where $$k$$ is
the length of a string. However, we can prove that $$k$$ is proportional to
$$\log(n)$$ where $$n$$ is the total number of nodes in the tree. Hence it is not a
linear time data structure indeed.

Compare to hash, Patricia tree has the advantage of lower memory consumption. It
does not use multiples of total data sizes for storage, however, it is not as
compact as a plain array. Hence it is something in the middle of them.

My implementation is using bits on the raw string for index. Strings are assumed
to be null-terminated array of characters. The code can be obtained here:
[header file](/img/patricia.h), [C++ source](/img/patricia.cc)
...
PS: The earlier version of the above program was in plain C. You can obtain the
tarball of the code [here](/img/patricia.tgz).
