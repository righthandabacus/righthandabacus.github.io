...
Solving this dual problem gives $$\alpha_i$$, which we can find the optimal $$w^{\ast} = \sum_i\alpha_iy_ix_i$$. To find the offset $$b$$ of the hyperplane, we have for each $$(x_i, y_i)$$,
